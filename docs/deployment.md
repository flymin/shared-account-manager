...
| `LOGIN_RATE_PER_MINUTE` | `30` | 所有 API 实例共享的登录校验预算补充速率，每分钟1–120次 |
| `LOGIN_BURST` | `10` | 登录预算可积累的突发额度，1–20次 |
...
所有登录请求在密码计算前检查共享预算，包括不存在的用户名和成功登录。默认每2秒补充1次校验额度，最多积累10次；同一数据库同时最多执行1次登录密码校验。并发或预算超限返回 `429` 与 `Retry-After` 秒数，不排队等待 Argon2，也不创建失败计数器。重建 API 容器不会重置已提交的预算。

同一用户名在15分钟窗口内失败10次后暂停校验，成功登录只清除该用户名的失败计数。全表最多新建到1024条，不淘汰仍有效的计数器来给随机用户名腾位置。过期计数器通过时间索引分批删除，登录入口和 worker 均能清理；worker 停止时，入口仍保持容量限制。升级前已有的大量计数器会逐批清理，达到容量上限时先拒绝新键，保留原有账号的限制。

这些限制保护密码计算和数据库存储，不依赖用户提交的 IP 或代理头。全站预算用尽时，正常用户的新登录也需要稍后重试；已有会话和账号业务操作不消耗登录预算。更大规模的入口流量应由部署环境按可信来源另行控制。提高预算前应评估宿主机资源，并保持所有 API 实例的配置一致。登录限流实现和回归范围见[开发指南](development.md#登录限流)。
